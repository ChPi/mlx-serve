...
- **Growth policy (ENFORCED)**: root CLAUDE.md must stay under the byte cap in `tests/test_claude_md_size.sh` (CI-gated). New gotcha = 1–3-line rule in `## Rules` + full story appended to the matching `docs/gotchas/*.md`. New subsystem = one Layout row + a `docs/reference.md` section. App-side content → `app/CLAUDE.md`. Never paste war stories or measurements here — link them.
...
| `server.zig` | All HTTP: `/v1/*` (chat/completions/messages/responses/embeddings/load/unload), media endpoints, `/metrics(.json)`, WS, Ollama glue, `--api-key` gate, built-in console at `GET /` (`src/html/`: skeleton + `app.css`/`app.js`/`metrics.js` injected as runtime `{s}` args, renders with NO model). Embeddings: BERT + bidirectional EmbeddingGemma + per-checkpoint pooling (mean|cls|last_token) + `--embedding-max-length`; full contract in `docs/reference.md` server.zig row |
...
- **`--model-dir` is REPEATABLE**: `discoverModelsMany` merges N roots FIRST-WINS (un-deduped merge fails registry init); app `ModelRoots` is the ONE answer to downloads + scan; DevID-gated (MAS helper can't inherit scoped grants). Second bite (2026-08-08): app-side READS (picker, `existingModelDir`, `resolveModelDir`, `componentReady`, delete scoping) go through `ownedRoots` = destination + built-in root, first-wins — `modelsDir` alone hid the pre-move library from the app while the server kept serving it; writes + cancel cleanup stay destination-pinned, test-pinned roots stay alone. Guards: `tests/test_multi_model_dir.sh`, `ModelRootsTests`, `OwnedRootDiscoveryTests`.
