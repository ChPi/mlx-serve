...
- **Long prompts on Flash Next prefill and decode faster.** The sparse-attention work that only depends on the context length is built once per forward instead of once per layer, speculative verification reads long contexts through one fused split-K kernel (8-13% faster verify rounds at 62k), block selection is one exact dispatch, and the 32 GB n-gram table's long-prompt reads go through the decode prefetch pool once the prompt is long enough to need it.
...
- The DFlash drafter no longer gives up so easily: it samples 8 rounds before deciding a request is not worth drafting for, instead of 3.
- Raising `iogpu.wired_limit_mb` no longer goes unheard: a 448k-token Flash Next conversation was refused an append because the guard sized itself against the RAM other applications left free (~100 GB) rather than the 120 GB limit the operator set. The raised limit is now a floor, 8 GB under it. Other models are unchanged.
- A warm Flash Next turn was billed for its whole cached prompt again on every turn that extended it, so a 448k session was refused an append of 73 tokens (13,450 MB asked, 12,934 MB free). A warm turn is now billed what it allocates: the same turn asks 7,051 MB.
- A Metal working-set out-of-memory during a very long prefill killed the server instead of failing one request (#353). The failing request now gets a 503 and the next one is served; the streaming surfaces return the same 400/503 as the non-streaming ones instead of a bare error frame.
- A long prefill was refused, or ran out of memory, for capacity it was never going to need twice (#353). Past 32k tokens a request reserves its attention cache up front, an omitted `max_tokens` no longer reserves the whole remaining context, and a prompt that only fits once the prefix cache gives memory back is admitted and the cache evicted, instead of refused.
- Long-context servers prefilled every prompt at the narrowest chunk. With `--ctx-size 1048576` the width was sized once at load against the whole session; on Flash Next it is now chosen per request and re-chosen at every chunk boundary.
- The prefix-cache budget depended on how busy the Mac was at startup, shrank when the GPU memory limit was raised, and a bigger `--prefix-cache-mem` could buy a smaller cache or an advertised context of 870 tokens. The load-time bills now read the static memory ceiling, a constant sizing reserve and the boot's `--kv-quant`.
...
- `--max-tokens N` in serve mode sets the reply budget for clients that omit the field.
- A refused turn on Flash Next could destroy the cached conversation it was handed, and a cached conversation could be pinned by a request that shared eleven tokens with it. A restored prefix is handed over only once the request is admitted and about to write; a restore that delivers almost nothing of what it pins is declined; the refusal names the credit it compared.
- `mlx-serve launch claude` drove a long-context server as a 200k one; the launcher now passes the advertised context.
- The server log said clients asked for a billion tokens when they omitted `max_tokens`; it now says auto. Flash Next's draft-head load line no longer calls its wiring pending, its per-layer debug switches are read once, and the n-gram warm-up says what it is doing.
